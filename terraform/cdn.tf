resource "aws_cloudfront_distribution" "cdn" {
    ...
    aliases             = var.primary_domain_list
...
resource "aws_cloudfront_distribution" "archive_cdn" {
    origin {
        origin_id   = local.s3_bucket_origin
        domain_name = resource.aws_s3_bucket.archive.bucket_regional_domain_name
    }

    aliases             = var.archive_domain_list
    default_root_object = "index.html"
    enabled             = true
    is_ipv6_enabled     = true
    price_class         = "PriceClass_100"  # NA and EU only (USA, Canada, Mexico, Europe, Israel).
